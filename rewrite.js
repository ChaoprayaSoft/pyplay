...
let content = fs.readFileSync('arduino_app.js', 'utf-8');

// 1. Replace pinCoordinates
const newPinCoords = `const pinCoordinates = {
    "pin-13": { x: 217, y: 89 },
    "pin-12": { x: 226, y: 89 },
    "pin-11": { x: 235, y: 89 },
    "pin-10": { x: 244, y: 89 },
...
content = content.replace(/const pinCoordinates = \{[\s\S]*?\};/, newPinCoords);

// 2. Replace renderWiresAndComponents
const newRender = `function renderWiresAndComponents(lesson) {
...
content = content.replace(/function renderWiresAndComponents\(lesson\) \{[\s\S]*?\}(?=\n\n\/\/ Set up event interactions)/, newRender);

// 3. Update Lessons array
const lessonsRegex = /const lessons = \[([\s\S]*?)\];\n\nlet currentLessonIndex/;

// Lesson 1: Blink (no wires/components)
// Lesson 2: External LED
const l2 = `wires: [
...
// Execute replacements for lessons manually
content = content.replace(/wires: \[\s*\{\s*from: "pin-9"[\s\S]*?\],\s*components: \[[^\]]*\]/m, l2);
content = content.replace(/wires: \[\s*\{\s*from: "pin-2"[\s\S]*?\],\s*components: \[[^\]]*\]/m, l3);
content = content.replace(/wires: \[\s*\{\s*from: "pin-a0"[\s\S]*?\],\s*components: \[[^\]]*\]/m, l5);
content = content.replace(/wires: \[\s*\{\s*from: "pin-a1"[\s\S]*?\],\s*components: \[[^\]]*\]/m, l6);
content = content.replace(/wires: \[\s*\{\s*from: "pin-7"[\s\S]*?\],\s*components: \[[^\]]*\]/m, l7);
content = content.replace(/wires: \[\s*\{\s*from: "pin-6"[\s\S]*?\],\s*components: \[[^\]]*\]/m, l8);
content = content.replace(/wires: \[\s*\{\s*from: "pin-4"[\s\S]*?\],\s*components: \[[^\]]*\]/m, l9);
content = content.replace(/wires: \[\s*\{\s*from: "pin-12"[\s\S]*?\],\s*components: \[[^\]]*\]/m, l10);

fs.writeFileSync('arduino_app.js', content, 'utf-8');
console.log('Successfully updated arduino_app.js!');
